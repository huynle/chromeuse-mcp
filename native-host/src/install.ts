...
/**
 * ChromeUse MCP - Native Host Manifest Installer
 *
 * Writes com.chromeuse.mcp_bridge.json to NativeMessagingHosts directories
...
export const NATIVE_HOST_NAME = 'com.chromeuse.mcp_bridge'
export const MANIFEST_FILENAME = `${NATIVE_HOST_NAME}.json`

/** Config directory for ChromeUse wrapper scripts */
const CONFIG_DIR = join(homedir(), '.chromeuse')
...
export function buildManifest(options: ManifestOptions): Record<string, unknown> {
  ...
  return {
    ...
    description: 'ChromeUse MCP Native Host',
...
# ChromeUse MCP - Native Host wrapper script
# Generated by chromeuse-native-host installer - do not edit manually
exec ${command}
`
...
async function main(): Promise<void> {
  ...
  const envIds = process.env.CHROMEUSE_EXTENSION_IDS
  if (envIds) {
  ...
  if (extensionIds.length === 0) {
    console.warn(
      ...
      'Pass extension IDs via --extension-id=<id> or CHROMEUSE_EXTENSION_IDS env var.',
    )
  }
